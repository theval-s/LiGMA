...
constexpr std::string GAME_WORK_DIR = ".work_game";
constexpr std::string MERGED_DIR = "game";
constexpr std::string UPPER_DIR = "upper";
constexpr std::string MOD_FILES_DIR = "mod_files";
constexpr std::string MODS_WORK_DIR = ".work_mods";
class InstanceFilesystem {
  ...
  public:
    ...
    InstanceFilesystem(const std::string &instanceName,
                       const fs::path &basePath, const fs::path &gamePath,
    ...
    const std::vector<std::pair<std::string, fs::path>> &getModList() const {
        return m_modList;
    }
    ...
    /// Gets all mod dirs from modList and returns a string with format
    /// dir1:dir2:...:dirn
