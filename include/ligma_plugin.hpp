...
public:
    ...
    virtual std::string executeCommand() const = 0;//currently just for executable name,
    //^ should be restructurized for customizable args probably

    virtual std::string modPath() const = 0;//in case mods should be stored somewhere else
    ...
    virtual void initializeModState(std::filesystem::path modsDirPath) const = 0; //Actions to start out modding of the game
    // like installing modding API, specific mods, setting up file structure in Ligma mod folder
    // path to Ligma _mods folder that gets mounted upon game folder is provided.
