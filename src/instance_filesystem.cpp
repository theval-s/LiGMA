...
namespace LigmaCore {
                       ...
                       (sanitizeForPath(instanceName) + ".json"))
    {
        ...
        fs::create_directory(basePath, basePath.parent_path()); // to copy permissions
        ...
        std::cerr << std::format("InstanceFilesystem initialized with basePath = "
                                 "{}\ngamePath = {}\nInstance Name = {}\n",
                                 basePath.string(), gamePath.string(), instanceName);
    ...
    InstanceFilesystem::InstanceFilesystem(const QJsonObject &config,
        ...
        const std::vector<QString> requiredKeys = {
            "instanceName", "pluginUUID", "basePath", "gamePath", "mounted", "modList"
        };
        for (const auto &k: requiredKeys) if (!config.contains(k)) throw std::runtime_error(
            "missing JSON value: " + k.toStdString());
        ...
        if (!mod_list_array.isEmpty()) {
            //rewrite this loop
            for (auto iter = mod_list_array.begin(); iter != mod_list_array.end(); ++iter){
            //for (const auto &item: mod_list_array) {
                ...
                if (mod_obj.contains("name") && mod_obj.contains("path")) {
                    m_modList.emplace_back(mod_obj["name"].toString().toStdString(),
                                           fs::path(mod_obj["path"].toString().toStdString()));
                } else throw(std::runtime_error("Errors in JSON parsing, modList array entry does not have name or path keys"));
            }
        }
    ...
    void InstanceFilesystem::addMod(const fs::path &modPath,
        ...
        fs::path dest_folder = m_basePath / MOD_FILES_DIR / sanitizeForPath(modName);
        ...
        try {
            ...
            std::cerr << "filesystem.cpp: directory iterator exception: " << e.what()
                    << std::endl;
            return;
        }
    ...
    std::string InstanceFilesystem::getLowerDirsString() {
        ...
        return mod_folders;
        //rightmost dir is the lowers layer, leftmost is the highest, so order is reversed and game is added on top
    }

    void InstanceFilesystem::removeMod(const size_t &id) {
        if (gameMounted) {
            unmount(m_basePath / MERGED_DIR);
        }
    ...
    void InstanceFilesystem::openGame() {
        ...
        const std::string command = std::format(
            "./{}", m_gamePlugin->executeCommand());
        system(command.c_str());
    }

    void InstanceFilesystem::unmount(const fs::path &dir) {
        gameMounted = false;
        saveState();
    }
    ...
    void InstanceFilesystem::cleanState() {
        if (gameMounted) unmount(m_basePath / MERGED_DIR);
        std::cerr << std::format(
            "InstanceFilesystem::cleanState() with base path <{}> removed {} files\n",
            m_basePath.string(), fs::remove_all(m_basePath));
    }

    void InstanceFilesystem::saveState() const { ConfigManager::saveInstance(m_configPath, this->toJson()); }
} // namespace LigmaCore
