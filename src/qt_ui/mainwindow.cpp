...
void MainWindow::RefreshLists() {
    std::vector<LigmaCore::PluginInfo> info = LigmaCore::PluginHandler::getInstance().getPluginInfo();
...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->pluginsList->selectionModel(), &QItemSelectionModel::selectionChanged,
        this, &MainWindow::pluginSelectionCheck);
    connect(ui->instanceList->selectionModel(), &QItemSelectionModel::selectionChanged,
        this, &MainWindow::instanceSelectionCheck);
...
void MainWindow::pluginSelectionCheck(const QItemSelection &selected, const QItemSelection &deselected) {
    if (selected.isEmpty()) ui->createInstanceButton->setDisabled(true);
    else ui->createInstanceButton->setDisabled(false);
}

void MainWindow::instanceSelectionCheck(const QItemSelection &selected, const QItemSelection &deselected) {
    if (selected.isEmpty()) ui->openInstanceButton->setDisabled(true);
    else ui->openInstanceButton->setDisabled(false);
}
...
void MainWindow::on_createInstanceButton_clicked() {
    if (const auto current_selected = ui->pluginsList->currentItem()) {
        ...
        std::cerr << "I am in selected plugin with index " << plugin_index << std::endl;
        std::vector<LigmaCore::PluginInfo> info = LigmaCore::PluginHandler::getInstance().getPluginInfo();
        //maybe i should store it?
        NewInstanceDialog dialog(info[plugin_index], this);
        dialog.setWindowTitle(
            QString::fromStdString(std::format("Creating instance for: {}", info[plugin_index].name)));
        if (dialog.exec() == QDialog::Accepted) {
            ...
            GameInstance *instance = new GameInstance(dialog.getInstanceName(),
                                                      dialog.getInstancePath(),
                                                      dialog.getGamePath(),
...
void MainWindow::on_openInstanceButton_clicked() {
    ...
    if (const auto current_selected = ui->instanceList->currentItem()) {
        ...
        std::filesystem::path conf_path = LigmaCore::ConfigManager::getConfigPath()/
            current_selected->text().toStdString();
        const QJsonObject& conf = (LigmaCore::ConfigManager::loadInstance(conf_path));
