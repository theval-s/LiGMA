...
NewInstanceDialog::NewInstanceDialog(const LigmaCore::PluginInfo &info, QWidget *parent)
    : QDialog(parent)
      , ui(new Ui::NewInstanceDialog) {
    ui->setupUi(this);
    connect(ui->gamePathLineEdit, &QLineEdit::textChanged, this, &NewInstanceDialog::validateInputs);
    connect(ui->nameLineEdit, &QLineEdit::textEdited, this, &NewInstanceDialog::validateInputs);
    //connect(ui->instancePathLineEdit, &QLineEdit::textEdited, this, &NewInstanceDialog::validateInputs);
    //move lower stuff to instance name line edit change signal
    _instancePath = QString::fromStdString(
        (std::filesystem::canonical("/proc/self/exe").parent_path() / "game_instances").string());
...
void NewInstanceDialog::validateInputs() {
    if (!ui->gamePathLineEdit->text().isEmpty() && !ui->nameLineEdit->text().isEmpty()
...
void NewInstanceDialog::on_gamePathBrowseButton_clicked() {
    QString game_dir = QFileDialog::getExistingDirectory(this, "Choose game directory");
    if (!game_dir.isEmpty()) {
...
void NewInstanceDialog::on_nameLineEdit_textEdited(const QString &arg1) {
    ui->instancePathLineEdit->setText(_instancePath + "/" + sanitizeForPath(arg1));
}
